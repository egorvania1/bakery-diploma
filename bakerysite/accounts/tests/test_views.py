...
class TestAccountLoginPage(TestCase):
    ...
    def test_login_can_not_login_with_bad_creds(self):
        wrong_credentials = {
            'username': 'nonecust',
            'password': 'passwordforcustomer'}
        response = self.client.post(reverse("accounts:login"), wrong_credentials, follow=True)
...
class TestAccountRegisterPage(TestCase):
    ...
    def test_register_can_register(self):
        response = self.client.post(reverse("accounts:register"), self.credentials, follow=True)
        try:
    ...
    def test_register_can_not_register_duplicate(self):
        user_c = User.objects.create_user(username='testcust', password='passwordforcustomer')
        ...
        self.assertFormError(user_form, 'username', 'Пользователь с таким именем уже существует.')
        self.assertFormError(profile_form, 'phone', 'Пользователь с таким номером телефона уже существует.')
...
class TestAccountProfilePage(TestCase):
    def setUp(self):
        ...
        self.user_c2 = User.objects.create_user(
            username="testcust2", password="passwordforcustomer2", first_name="testcust2"
        )
